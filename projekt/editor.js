window.onload = function () {
    ...
    if (typeof angular !== 'undefined' && typeof showdown !== 'undefined') {
      (function (module, showdown) {
        ...
        function ngShowdown() {
          ...
          function SDObject() {
            let converter = new showdown.Converter(config);
        ...
        function getLinkFn($showdown, $sanitize, $sce) {
          return function (scope, element, attrs) {
            scope.$watch('model', function (newValue) {
              ...
              // if (sesssionStorage.getItem("mainText")) {
              //   let mText = sessionStorage.getItem("mainText");
    ...
    let app = angular.module('showdown.editor', ['ng-showdown', 'pageslide-directive', 'ngAnimate', 'ngRoute', 'ngCookies', 'ngSanitize']);
    ...
    app.controller('editorCtrl', ['$scope', '$showdown', '$http', '$cookies', '$sanitize', function ($scope, $showdown, $http, $cookies, $sanitize) {
      ...
      let text = '';
      let savedCheckOpts = $cookies.getObject('checkOpts') || [];
      let savedNumOpts = $cookies.getObject('numOpts') || [];
      let savedTextOpts = $cookies.getObject('textOpts') || [];
      let defaultOpts = $showdown.getDefaultOptions(false);
      let checkOpts = {
      ...
      if (defaultOpts !== null) {
        for (let opt in defaultOpts) {
          if (defaultOpts.hasOwnProperty(opt)) {
            let nOpt = (defaultOpts[opt].hasOwnProperty('defaultValue')) ? defaultOpts[opt].defaultValue : true;
      ...
      for (let i = 0; i < $scope.checkOpts.length; ++i) {
          for (let ii = 0; ii < savedCheckOpts.length; ++ii) {
      ...
      if (window.location.hash) {
        ...
        let defHtml = $http.get('md/text.md');
        defHtml
          .then(function(res) {
            $scope.text = res.data;
